...
import { SearchResult, MetricResult } from '@/types'

interface MetricsDisplayProps {
  result: MetricResult
}

interface QueryMetricsDisplayProps {
  result: SearchResult
}
...
export function QueryMetricsDisplay({ result }: QueryMetricsDisplayProps) {
  const metrics = [
    ...
    { label: 'RR@k', value: result.metrics.mrr_at_k },
    { label: 'Avg Popularity', value: result.metrics.pop_at_k },
  ]

  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-lg font-semibold mb-4 text-gray-900">Query Metrics</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {metrics.map((metric) => (
          <div key={metric.label} className="text-center">
            <div className="text-2xl font-bold text-blue-600">
